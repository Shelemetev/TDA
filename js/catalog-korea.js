...
dataKorea.map(item => {
    document.querySelector('.catalog__swiper-wrapper--korea').innerHTML += `
        <div class="swiper-slide catalog__slide catalog__slide--korea" data-count="${item.count}">
...
var swiper = new Swiper(".catalog__swiper--korea", {
    spaceBetween: 16,
    slidesPerView: 4,
    allowTouchMove:false,
    navigation: {
        nextEl: ".catalog__slide-button-next--korea",
        prevEl: ".catalog__slide-button-prev--korea",
    },
});

// let items = {...}

document.querySelectorAll('.catalog__btn--korea').forEach(btn => {
    btn.addEventListener('click', () => {
        document.querySelectorAll('.catalog__btn--korea').forEach(btnRemove => {
            btnRemove.classList.remove('catalog__btn--active')
        })
        btn.classList.add('catalog__btn--active')
        document.querySelectorAll('.catalog__slide--korea').forEach(item => {
